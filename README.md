...
- Notes 筆記：資料夾與巢狀資料夾、所有筆記第一層資料夾、非選單資料夾長按操作、未分類、垃圾桶、置頂分區、搜尋/篩選、排序/檢視下拉切換、長按直接進入固定批量編輯列、四種模板入口、一般筆記 Rich Text 即時格式化編輯器、toolbar/文本分離、復原/再製歷史、筆記內真實核取方塊、#標籤、圖片/附件依插入位置顯示於文本區、圖片獨立行佔位、圖片外框貼合實際圖片比例、鍵盤刪除圖片時同步清除附件資料、自由移動圖片可在整個圖片行重新點選/拖移、圖片裁切/縮放/框線/對齊/刪除工具列、附件開啟/下載/刪除工具列、左滑刪除。
...
- UI：下方導覽列套用 Nav Bars Template 6 風格，並保留未來切換導覽列風格的程式結構；首頁與筆記頁 FAB 選單採放大藥丸外框、由下往上 0.1 秒分段滑入動畫；存餘帳戶新增/編輯維持原本彈窗樣式。
